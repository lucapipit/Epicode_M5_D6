...
function SingleBook({ asin, img, title, price, category}) {
    ...
    return (
        <Card className="p-0 m-2" style={{ maxWidth: "18rem" }}>
            <div style={{ height: "450px" }}>
                <Card.Img variant="top" src={img} style={{ height: "100%" }} />
            </div>
            <Card.Body className={theme ? "lightTheme myCard" : "darkTheme myCard"}>
                ...
                onClick={()=>{
                    ...
                    }} style={{ textDecoration: "none", cursor: "pointer" }}> <i class="bi bi-chat-fill text-primary"></i> comments</a>
                <Link to={`bookDetail/${asin}/${title}/${price}/${category}`} style={{textDecoration: "none"}}>
                    <div className='text-success' onClick={dispatch(selectSingleBook(asin))} style={{cursor: "pointer"}}><i class="bi bi-eye-fill text-success"></i> detail</div>
                </Link>
